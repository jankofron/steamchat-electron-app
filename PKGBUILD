...
pkgname=steamchat-electron-app
pkgver=1.0.0
pkgrel=1
pkgdesc="Steam chat wrapper (Electron)"
arch=('x86_64')
url="https://github.com/jankofron/steamchat-electron-app"
license=('MIT')
depends=('electron' 'hicolor-icon-theme' 'libappindicator-gtk3')
makedepends=('nodejs' 'npm' 'asar' 'git')
source=("${pkgname}-${pkgver}.tar.gz::https://github.com/jankofron/steamchat-electron-app/archive/refs/tags/v${pkgver}.tar.gz")
...
#!/bin/sh
exec /usr/bin/electron /usr/lib/steamchat-electron-app/app.asar "$@"
EOF
...
[Desktop Entry]
Name=Steam Chat
Comment=Steam Chat Electron wrapper
Exec=steamchat-electron-app
Terminal=false
Type=Application
Categories=Utility;
Icon=steamchat-electron-app
EOF
